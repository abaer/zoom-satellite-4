...
class Sparkle extends Component {
  constructor(props) {
    ...
    // const minXGap = rangeData.reduce((data, pt) =>{
    //   const gap = pt.x - data.lastX
    //   const min = (data.min === undefined || gap < data.min) ? gap : data.min
    //   return {lastX:pt.x, min}
    // },{lastX:0, min:undefined})
    // const barWidth = (minXGap.min > 2) ? 3 : 3
    const barWidth = 2.5
    this.state = {
      rangeData,
      barWidth,
